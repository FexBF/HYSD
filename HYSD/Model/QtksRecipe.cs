...
namespace HYSD
{
    public partial class QtksRecipe : UserControl
    {
        // 定义初始样式和选中样式
        ...
        public QtksRecipe(ILogger logger, SqlSugarClient db, IOmronPlcService plc)
        {
            InitializeComponent();
        ...
        private void InitButtonGroup()
        {
            ...
            comboBox1.DataSource = _db.Queryable<QtksRecipeMain>().Select(c => c.RecipeName).ToList();
        ...
        private void ButtonGroup_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // 再基于常量 recipeId 查询子表
                var dataSet = _db.Queryable<QtksRecipeDataSet>()
                    .Where(c => c.RecipeID == recipeId && c.SetIndex == _currentIndex)
                    .First();

                var dataSets = _db.Queryable<QtksRecipeDataSet>()
                ...
                if (dataSets?.Count != 0)
                {
                    ...
                    foreach (var ds in dataSets)
                    {
                        if (ds.SetIndex != 0)
                        {
                            _labels[ds.SetIndex - 1].Text = ds.ATime.ToString();
                        }
                        if (ds.ATime > 0)
                        {
                ...
                if (dataSet != null)
                {
                    ...
                    numTextBox4.Text = dataSet.ATime.ToString();
                    ...
                    toggleSwitchs6.IsOn = dataSet.Pluse1SW;
                    numTextBox23.Text = dataSet.Pluse1Curr.ToString();
                    numTextBox21.Text = dataSet.Pluse1ONtime.ToString();
                    numTextBox25.Text = dataSet.Pluse1OFFtime.ToString();
                    toggleSwitchs7.IsOn = dataSet.Pluse2SW;
                    numTextBox22.Text = dataSet.Pluse2Curr.ToString();
                    numTextBox20.Text = dataSet.Pluse2ONtime.ToString();
                    numTextBox24.Text = dataSet.Pluse2OFFtime.ToString();
                }
                else
                {
                    // 如果没有数据集，清空界面上的控件
        ...
        {
            var recipeid = _db.Queryable<QtksRecipeMain>().Where(c => c.RecipeName == alphaNumTextBox1.Text).Select(c => c.RecipeID).First();
            ...
            try
            {
                QtksRecipeDataSet dataSet = new QtksRecipeDataSet();
                ...
                dataSet.ATime = Convert.ToUInt16(numTextBox4.Text);
                ...
                dataSet.Pluse1SW = toggleSwitchs6.IsOn;
                dataSet.Pluse1Curr = Convert.ToUInt16(numTextBox23.Text);
                dataSet.Pluse1ONtime = Convert.ToUInt16(numTextBox21.Text);
                dataSet.Pluse1OFFtime = Convert.ToUInt16(numTextBox25.Text);
                dataSet.Pluse2SW = toggleSwitchs7.IsOn;
                dataSet.Pluse2Curr = Convert.ToUInt16(numTextBox22.Text);
                dataSet.Pluse2ONtime = Convert.ToUInt16(numTextBox20.Text);
                dataSet.Pluse2OFFtime = Convert.ToUInt16(numTextBox24.Text);
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            {
                //创建配方
                _db.Insertable(new QtksRecipeMain
                {
                ...
                var ds = _db.Queryable<QtksRecipeMain>().Select(c => c.RecipeName).ToList();
        ...
        private void pictureBox2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var recipeId = _db.Queryable<QtksRecipeMain>()
                    ...
                    .First(); // 返回 int，若无记录可能为 0
                // 删除关联的子表数据
                _db.Deleteable<QtksRecipeDataSet>().Where(ds => ds.RecipeID == recipeId).ExecuteCommand();
                _db.Deleteable<QtksRecipeMain>().Where(c => c.RecipeName == alphaNumTextBox1.Text).ExecuteCommand();
        ...
        private void rjButton16_Click(object sender, EventArgs e)
        {
            ...
            if (_db.Queryable<QtksRecipeMain>().Where(c => c.RecipeName == alphaNumTextBox1.Text).Select(c => c.RecipeID).First() == 0)
            {
                ModernMessageBox.Show("当前配方不存在!请先创建并保存配方!");
                return;
            }
            try
            {
               ...
               .First(); // 返回 int，若无记录可能为 0
                ...
                var dataSet = _db.Queryable<QtksRecipeDataSet>().Where(c => c.RecipeID == recipeId && c.SetIndex == Convert.ToUInt16(numTextBox14.Text))
                    .First();
                ...
                if (dataSet != null && _plc != null && _plc.IsConnected)
                {
                    ...
                    OperateResult result = _plc.WriteCustomer($"D{434 + 34 * (dataSet.SetIndex - 1)}", data);
                    if (result.IsSuccess)
                    {
            ...
            try
            {
               ...
               .First(); // 返回 int，若无记录可能为 0
                ...
                if (dataSets != null && _plc != null && dataSets.Count != 0 && _plc.IsConnected)
                {
                    ...
                    //下载总序
                    _plc.Write("D946", Convert.ToUInt16(numTextBox15.Text));
        ...
        {
            AutofacConfig.isQtksChange = true;
        }
    }
}
